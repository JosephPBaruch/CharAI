...
**ephemeral** and must not be committed to the repository. The CI workflow trains
a fresh model before every Playwright E2E run so that tests always execute
against an up-to-date model, but the resulting file is discarded when the
workflow finishes.

- **CI usage**: The `ci.yaml` workflow calls the training script inside the
  Docker container before Playwright tests. The model exists only for the
  lifetime of that workflow run.
- **Production / curated models**: Must be created and managed manually outside
  of CI. Store them according to your deployment process; do not check them into
  version control.
