import { Box, Stack, Typography, Accordion, AccordionSummary, AccordionDetails, TextField, Select, MenuItem, InputAdornment } from '@mui/material';
...
export default function FieldsList({ field, onUpdateField }: FieldsListProps) {
  ...
  return (
    <Box>
      <Box sx={{ mb: 2 }}>
        <Typography variant="h6" sx={{ color: COLORS.whiteHigh, fontWeight: 600, mb: 0.5 }}>
          Field Configuration
        </Typography>
        <Typography variant="body2" sx={{ color: COLORS.whiteMedium }}>
          Configure your field's crop type and current selling price to calculate biochar ROI.
        </Typography>
      </Box>
      ...
      >
        ...
        >
          <Typography sx={{ color: COLORS.whiteHigh, fontWeight: 600, flex: 1 }}>
            {field.cropType === 'Other' ? (field.customCrop || 'Other') : field.cropType}
          </Typography>
          {field.price && (
        ...
        <AccordionDetails>
          <Stack direction={{ xs: 'column', lg: 'row' }} spacing={2} alignItems="stretch">
            ...
            <Stack spacing={1} sx={{ minWidth: 220, flex: 1 }}>
              ...
              {field.cropType === 'Other' && (
                <TextField
                  ...
                  onChange={(e) => onUpdateField({ customCrop: e.target.value })}
            ...
            <Stack spacing={1} sx={{ minWidth: 250, flex: 1 }}>
              <Typography variant="subtitle2" sx={{ color: COLORS.whiteHigh, fontWeight: 500 }}>
              ...
              <Stack direction="row" spacing={1} alignItems="center">
                <TextField
                  ...
                  error={!isPriceValid}
                  value={field.price}
                  onChange={(e) => onUpdateField({ price: e.target.value === '' ? '' : Number(e.target.value) })}
                  sx={{
                    minWidth: 120,
                    '& .MuiOutlinedInput-root': {
                      color: COLORS.whiteHigh,
                      '& fieldset': { borderColor: !isPriceValid ? COLORS.error : COLORS.whiteLow },
                      '&:hover fieldset': { borderColor: !isPriceValid ? COLORS.error : COLORS.indigo },
                    },
                    '& .MuiInputLabel-root': {
                ...
                <Select
                  value={field.unit}
                  onChange={(e) => onUpdateField({ unit: e.target.value as 'ton' | 'kg' | 'bushel' })}
                  size="small"
                  sx={{
                    color: COLORS.whiteHigh,
                    '& .MuiOutlinedInput-notchedOutline': { borderColor: COLORS.whiteLow },
